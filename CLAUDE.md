...
**Collision layer/team convention:** physics layer 1 = `world`, layer 2 = `player_collision`. Hitboxes use `collision_layer = 0, collision_mask = 3`; Hurtboxes use `collision_layer = 3, collision_mask = 0`. Friendly-fire is prevented by comparing `Attack.team` against the target's `team` string (`"player"` / `"enemy"`), **not** by physics layers — so an attack only matters if its `team` differs from the victim's.
